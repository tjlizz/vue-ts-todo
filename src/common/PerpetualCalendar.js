(function () {
    ...
    var DetailsDay = function (isToday, sYear, sMonth, sDay, week, lYear, lMonth, lDay, isLeap, cYear, cMonth, cDay) {
        this.isToday = false;//是否当天
    ...
    var DetailsMonth = function (year, month) {



    }

    var SolarDay = function (year, month, day, week, isWeekend) {
        this.year = year;
        this.month = month;
        this.day = day;
        this.week = week;//星期几
        this.isWeekend = isWeekend;//是否假期
    }
    var SolarMonth = function (year, month) {
        this.year = year;
        this.month = month;
        this.dayCount = solarDays(year, month);//当月天数
        this.days = [];     
        var firstDay = new Date(y, m, 1); //当月第一天
        this.firstWeek = firstDay.getDay();//获取当月第一天星期几

        for (var i = 0; i < this.dayCount; i++) {            
            if ((i + this.firstWeek) % 7 == 0 || (i + this.firstWeek) % 7 == 6) {
                this.days.push(new SolarDay(y, m, i + 1, true));
            }
            else {
                this.days.push(new SolarDay(y, m, i + 1,false));
            }
        }
...
"0730 被奴役国家周"];
    ...
    var calendar = function (y, m) {
        ...
        for (var i = 0; i < this.length; i++) {
            ...
            //cYear,cMonth,cDay
            this[i] = new LeadingDay(y, m + 1, i + 1, nStr1[(i + this.firstWeek) % 7],
            lY, lM, lD++, lL,
            cyclical(lDObj.yearCyl), cyclical(lDObj.monCyl), cyclical(lDObj.dayCyl++))
            if ((i + this.firstWeek) % 7 == 0) {
                this[i].isWeekend = true;
            }
        ...
        //节气
        tmp1 = getTerm(y, m * 2) - 1
        tmp2 = getTerm(y, m * 2 + 1) - 1
    ...
    LeadingCloud.PerpetualCalendar = {
        //传入年份得到天干地支
        cyclical: function (y) {
